...
function crExport(format) {
  ...
  if (format === "csv") {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>รายงาน</title><style>@import url('https://fonts.googleapis.com/css2?family=Sarabun:wght@400;700');body{font-family:Sarabun,sans-serif;padding:20px}table{width:100%;border-collapse:collapse;font-size:14px}th,td{border:1px solid #6c8074;padding:8px;text-align:left}th{background:#ebf0ec;font-weight:700}.hd{display:flex;align-items:center;gap:14px;border-bottom:2px solid #16241b;padding-bottom:10px;margin-bottom:12px}</style></head><body><div class="hd">${logoImgTag(44)}<div><h2 style="margin:0">รายงานสต๊อกคงเหลือ ❄️ SQF</h2><p style="margin:2px 0 0;font-size:12px;color:#6c8074">อัปเดต: ${new Date().toLocaleString("th-TH")}</p></div></div><table><thead><tr><th>ชื่อสินค้า</th><th>MFG</th><th>จำนวน</th><th>เหลือ(วัน)</th><th>สถานะ</th><th>QC</th></tr></thead><tbody>${sorted.map(r=>`<tr><td>${escapeHtml(r.ProductName)}</td><td>${isoToDdmmyy(r.MFG)}</td><td>${r.Qty} ${escapeHtml(r.Unit)}</td><td>${r.ExpireDays}</td><td>${r.ExpireStatus}</td><td>${r.QcShelfLifeStatus}</td></tr>`).join("")}</tbody></table></body></html>`);
